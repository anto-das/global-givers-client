    ...
    return (
      <div className='flex justify-center items-center min-h-[calc(100vh-306px)]'> 
            <div className="p-2 md:p-6 mx-auto bg-white rounded-md shadow-lg  space-y-4">
              ...
              <input id='thumbnail' name='thumbnail' defaultValue={thumbnail}  type="url" className="input py-2 text-lg w-full" placeholder="Thumbnail" required />
              </div>
              {/* post title field*/}
                <div>
                <label className="label text-lg" htmlFor='post-title'>Post Title</label>
                <input id='post-title' name='post_title' defaultValue={postTitle} type="text" className="input text-lg w-full" placeholder="Post Title" required/>
                </div>
              {/* location field*/}
                <div>
                <label className="label text-lg" htmlFor='location'>Location</label>
                <input id='location' name='location' defaultValue={location} type="text" className="input text-lg w-full" placeholder="Location" required/>
                </div>
              {/* category field*/}
                <div>
               ...
               <select 
                ...
                <input id='volunteer' name='volunteer_number' defaultValue={volunteersNeeded} type="number" className="input text-lg w-full" placeholder="number of volunteer" required/>
                </div>
                  </div>
                  {/* description field */}
                   <fieldset className="fieldset">
        <legend className="label text-lg">Description</legend>
        <textarea className="textarea h-24 w-full" defaultValue={description} name='description' placeholder="Text your details" required></textarea>
      </fieldset>
                <button className="btn border-none shadow-none bg-[#29AF8A] rounded-none text-white text-lg hover:btn hover:rounded-none hover:text-white hover:bg-[#535353] hover:text-lg hover:border-none hover:shadow-none w-full mt-5">Update Post</button>
              </form>
            </div>
              </div>
    );
